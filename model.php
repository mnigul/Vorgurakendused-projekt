...
function model_load($page) {
	...
	$query = 'SELECT Id, Nimetus, Kogus FROM mnigul__kaubad
...
function model_add($nimetus, $kogus) {
	...
	$query = 'INSERT INTO mnigul__kaubad (Nimetus,Kogus) VALUES(?,?)';
...
function model_delete($id) {
	...
	$query = 'DELETE FROM mnigul__kaubad WHERE id=? LIMIT 1';
...
function model_update($id, $kogus) {
	...
	$query = 'UPDATE mnigul__kaubad SET Kogus=? WHERE Id=? LIMIT 1';
	$stmt = mysqli_prepare($l, $query);
	if( mysqli_error($l) ){
		echo mysqli_error($l);
		exit;
...
function model_user_add($kasutajanimi, $parool) {
	...
	$query = 'INSERT INTO mnigul__kasutajad (Kasutajanimi, Parool) VALUES (?,?)';
...
function model_user_get($kasutajanimi, $parool) {
	...
	$query = 'SELECT Id, Parool FROM mnigul__kasutajad
